...
def preprocess_image(image_path, target_size=(256, 256)):
    ...
    img = img.resize(target_size)
    img_array = np.array(img)
    if len(img_array.shape) == 2:  # If it's a grayscale image
        img_array = np.stack((img_array,)*3, axis=-1)
    elif img_array.shape[-1] == 4:  # If it's an RGBA image
        img_array = img_array[..., :3]
    img_array = img_array / 255.0
    img_array = np.expand_dims(img_array, axis=0)
    return img_array
...
def calculate_solar_potential(latitude, longitude, area=None, image_path=None):
    logger.debug(f"Calculating solar potential for lat={latitude}, lon={longitude}, area={area}, image_path={image_path}")
    ...
    if image_path:
        try:
            ...
            post_processed_mask = post_process_mask(binary_mask)
            total_building_area, image_area = calculate_total_area(image_path, post_processed_mask)
            logger.debug(f"Image processing results: total_building_area={total_building_area}, image_area={image_area}")
        except Exception as e:
            logger.error(f"Error processing image: {str(e)}")
            # If image processing fails, fall back to using the provided area
    ...
    logger.debug(f"Final areas: total_building_area={total_building_area}, image_area={image_area}")

    if total_building_area is None or image_area is None:
        logger.error("Failed to determine building area or image area")
        return {
            'error': 'Failed to determine building area or image area',
    ...
    try:
        ...
        logger.error(f"Values: total_building_area={total_building_area}, annual_irradiance={annual_irradiance}, panel_efficiency={panel_efficiency}, performance_ratio={performance_ratio}")
        return {
            'error': f'Error calculating solar potential: {str(e)}',
            'solar_potential': None,
...
if __name__ == "__main__":
    image_path = "D:\final_model\static\images\map_27.17659999886242_78.00870000000002.png"
    latitude = 40.7128
    longitude = -74.0060
